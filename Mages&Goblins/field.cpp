...
{
    if (units[row][col] == nullptr) return false;
...
bool Field::moveUnit(int srow, int scol, int trow, int tcol)
{
    if (units[srow][scol] == nullptr ||
        ...
        terrains[trow][tcol] != NULL)
        return false;
    //Change the unit
    units[srow][scol]->setRow(trow);
    units[srow][scol]->setCol(tcol);
    //Adapt Grid units.
    setUnit(trow, tcol, units[srow][scol]);
...
{
    assert (terrains.inBounds(row, col));//Assert fail
